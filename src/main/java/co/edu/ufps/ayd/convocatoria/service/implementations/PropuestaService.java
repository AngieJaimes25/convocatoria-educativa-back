...
public class PropuestaService implements PropuestaInterface {
    ...
    public List<PropuestaDTO> listarPropuestasAgrupadasPorTipo() {
        ...
        if (convocatorOptional.isEmpty()) {
            return Collections.emptyList(); // Devuelve una lista vacía si no hay convocatoria activa
        }
                ...
                .flatMap(entry -> {
                    ...
                    return propuestasPorTipo.stream()
                            .map(propuesta -> {
                                ...
                                propuestaDTO.setEvaluador(propuesta.getEvaluador());
                                return propuestaDTO;
                            });
                })
                .collect(Collectors.toList());
